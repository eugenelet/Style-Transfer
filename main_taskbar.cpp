...
// Variables to pass to callback function of trackbars
struct src_target
{
	Mat image;
	string identity;
...
int main(int, char** argv)
{
    ...
    invert_on = false;
  	...
  	// Prepare sturct that will be applied to callback funciton of taskbar
    ...
    /// Create Trackbars (source)
    char TrackbarName[50];
    sprintf( TrackbarName, "Dilation Threshold (Markers) x %d", 20 );
    createTrackbar( TrackbarName, "source", &thres_slider, 20, source_trackbar, &src_struct );
    sprintf( TrackbarName, "Partition Number x %d", 10 );
    createTrackbar( TrackbarName, "source", &partition_slider, 10, source_trackbar, &src_struct );
    sprintf( TrackbarName, "Invert on");
    createTrackbar( TrackbarName, "source", &invert_on, 1, source_trackbar, &src_struct );
    source_trackbar( thres_slider, &src_struct);
    /// Create Trackbars (target)
    sprintf( TrackbarName, "Dilation Threshold (Markers) x %d", 20 );
    createTrackbar( TrackbarName, "target", &thres_slider_t, 20, target_trackbar, &target_struct );
    sprintf( TrackbarName, "Partition Number x %d", 10 );
    createTrackbar( TrackbarName, "target", &partition_slider, 10, target_trackbar, &target_struct );
    target_trackbar( thres_slider_t, &target_struct );
...
void source_trackbar(int, void* ptr){
    ...
    // Store colors used for segmentation
    ...
    // Cut original image into multiple layers based on segmented results
...
// For target
void target_trackbar(int, void* ptr){
	Mat src = ((src_target*)ptr)->image;
	string w_name = ((src_target*)ptr)->identity;
    ...
    // Project original image into multiple layers
    Mat *segmentedPlanes = new Mat[partition_slider + 1];
    for(int i=0; i<partition_slider + 1; i++)
    	segmentedPlanes[i] = originalImg.clone();
    for(int i=0; i<partition_slider + 1; i++)
    ...
    // imshow("swaped image NO laplac", mergedSwap);
...
// Fills blank space (black) of segmented plane
void fillSegmentedPlane(Mat &plane){
	int mean_r = 0;
	int mean_g = 0;
	int mean_b = 0;
...
// Swap magniture and phase spectrum of two images
void swapMagPhase(Mat& dft1, Mat& dft2){
...
// 3 Channel swapping (maginute and phase spectrum) of 2 images
void perform_Swap(Mat &inputImage, Mat &inputImage2, Mat &mergedImage, Mat &mergedImage2){
...
// Partitions image into n big blobs
void partition(Mat src, Mat &markers, Mat &dst, vector<vector<Point> > &contours, vector<Vec3b> &colors){
